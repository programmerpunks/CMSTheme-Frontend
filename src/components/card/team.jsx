...
export const TeamCard = ({
  ...
  return (
    <>
      <div
        className={`card-image ${profileImage && "bg-transparent"} 
      ...
      >
        <label htmlFor="file-input">
          {member ? (
            <img
              src={member.image}
              className="profile-image bg-transparent"
              alt=""
            />
          ) : profileImage ? (
            <img
              className="profile-image bg-transparent opacity-25"
              alt=""
      ...
      <div className="card-body text-center">
        <div className="d-flex justify-content-center align-items-center mb-2">
          {(member ? member.stars.length === 0 : stars.length === 0) ? (
            <span style={{ color: `${colors.primary[200]}`, fontSize: 12 }}>
              Add reviews here
            </span>
          ) : (
            <ReviewStars
              setStars={setStars}
              stars={member ? member.stars : stars}
            />
          )}
          {!member && (
            <AddIcon
              className="add-button"
              onClick={() => addReview({ setStars, stars, editable })}
            />
          )}
        </div>
        ...
        <div className="d-flex justify-content-center">
          <Input
            ...
            editable={editable}
            fieldstyle="fw-bold mb-2 fs-6 mns-7"
            value={experience}
